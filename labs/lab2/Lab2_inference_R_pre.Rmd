...
```{r}
# read in the data
df_rain2 <- read_csv("labs/lab2/lab2_rainfall.csv")

# filter to just rainy days in 2001
df_rain2_rainy <- df_rain2 |> 
  filter(year == "2001",
         rain > 0)

# estimate the mean (mu_hat) and standard deviation (sigma_hat) of rainfall amount in rainy days
mu_hat = mean(df_rain2_rainy$rain)
sigma_hat = sd(df_rain2_rainy$rain)

cat('mu_hat is',round(mu_hat,2),'mm;',
    'sigma_hat is',round(sigma_hat,2),'mm')
...
print(bootstrap_standard_error(df_rain2_rainy))



# Repeat the point estimate and confidence interval estimate using rainy day data across all years
df_sub = df_rain2 |>
  filter(rain > 0)
...
cat('mu_hat is',round(mu_hat,2),'mm;',
    'sigma_hat is',round(sigma_hat,2),'mm;',
    'standard errors are',bootstrap_standard_error(df_sub))
...
```{r}
# get a new column with the rainy days
df_rain2 <- df_rain2 |> 
  mutate(is_rain = (rain > 0))

# estimate probability of rainfall event on each doy
df_rain_prob <- df_rain2 |> 
  group_by(doy) |> 
  summarise(prob = mean(is_rain))
